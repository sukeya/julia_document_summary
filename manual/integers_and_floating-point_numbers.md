...
64
```
Int、UIntはシステムのビット数桁の整数、符号なし整数である。
...
0
```
符号なし整数リテラルのサイズは先頭が0でないなら、そのリテラルを表すために必要な最小限のサイズである。
...
### 計算機イプシロン
まず、計算機イプシロンとは、1と1より大きい最小の浮動小数点数との差である。eps関数は引数に応じて以下の計算機イプシロンを返す。
- 型 ... その型での計算機イプシロンを返す。
...
### 任意精度計算
JuliaはGNU Multiple Precision Arithmetic Library(GMP)とGNU MPFR Libraryを使い、それぞれBigInt、BigFloatに対応している。
コンストラクターを使えば数値型からBigInt、BigFloatに変換できる。
```
> BigInt(1)
1
```
また、@big_strとparseを使えばAbstractStringsからも作れる。
```
> big"1234"
1234
...
1234
```
ただし、BigInt、BigFloatへの数値型の型プロモーションは行われず、明示する必要がある。

BigFloatのデフォルトの精度と丸め方をsetprecisionとsetroundingで変更できる。
また、精度と丸め方の変更を局所化できる : setprecision、setroundingの後にdo ~ endブロックを書けば、そのブロック内でのみ変更される。
```
...
### 数値リテラル係数
Juliaでは変数の直前に数値リテラルを書くことで、暗黙にその数を変数にかけることができる。
```
> x = 3;

> 2x^2 - 3x + 1, 1.5x^2 - 5x - 1, 2^2x
(10, -2.5, 64)
```
優先順位は単項演算子より下で、二項演算子より上である。ただし、べき乗演算子"^"は例外で、2x^3は(2x)^3、2^3xは2^(3x)と解釈される。
```
> x = 3;

> -2x, √2x, 2^3x, 2x^3
(-6, 4.242640687119286, 512, 54)
```
括弧があっても使える。
```
> 2(x - 1) ^ 2 - 3(x - 1) + 1
...
```
この2つ目の例では、()が関数呼び出しと解釈されたため、エラーが発生した。
3つ目の例では、xyが変数名と解釈されたためエラーが発生した。

### 文法の衝突
数値リテラル係数はE表記と2、8、16進数リテラルの文法と衝突するため、以下のルールがある。
- 0x/0o/0bで始まる表現は16/8/2進数リテラル。
- 数値リテラルの後にe、E、fが続く表現は浮動小数点数リテラル。

### 0と1のリテラル
以下の関数は数値の比較で不必要な型変換を避けるために便利である。
- zero(x) 型xか変数xの型の0のリテラルを返す。
- one(x) 上と同様
